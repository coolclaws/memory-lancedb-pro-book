...
+--------+    | -prompts  |    +----------+    +----+-----+
              ...
              |                |                    |
              v                v                    v
        [profile]     [pref/entity/pattern]   [events/cases]
              |                |                    |
              v                v                    v
        跳过向量预过滤    Stage 1: 向量预过滤     Stage 1: 向量预过滤
        始终合并到       (阈值 0.7, top-3)      (阈值 0.7, top-3)
        已有 profile          |                    |
              |                v                    v
              |         Stage 2: LLM 语义决策  Stage 2: LLM 决策
              |         7 种决策:              仅允许 create/skip
              |         create/merge/skip/     (不允许 merge)
...
六分类体系否定了两个极端方案。**三分类**（事实/事件/偏好）过于粗粒度，导致去重逻辑中充斥条件判断——用户名和公司名都是"事实"但去重策略截然不同。**十分类**（将 profile 拆为 name/role/background 等）虽然精确，但 LLM 分类准确率与类别数成反比：6 类时约 90%，10 类可能降到 75%。分类错误会导致后续合并策略完全走偏——一条 event 被错分为 preference 就会被错误地覆盖更新。此外每增加一个类别需要在 extraction-prompts.ts 中维护一整套提取/去重/合并 prompt，维护成本线性增长。六个类别在人类认知模型中也有根据——它大致对应认知科学中对长期记忆的经典分类。

...
```
+------------+     +----------+     +-----------+     +----------+
|  对话输入   +---->+ 智能提取  +---->+ 向量嵌入   +---->+ LanceDB  |
| (用户消息)  |     | Extractor|     | Embedder  |     | 存储写入  |
+------------+     +----+-----+     +-----------+     +-----+----+
                        |                                    |
                   分类 / 去重                           scope 标签
                        |                                    |
+------------+     +----v-----+     +-----------+     +-----v----+
|  检索结果   +<----+ 评分管线  +<----+ 混合检索   +<----+ 查询请求  |
| (Top-K)    |     | Scoring  |     | Retrieval |     | (recall) |
+------------+     +----------+     +-----------+     +----------+
```
...
```
memory-lancedb-pro 模块全景
├── 存储层 Storage
│   ├── store.ts ---------- LanceDB 读写封装
│   ├── embedder.ts ------- 多提供商向量嵌入
│   ├── chunker.ts -------- 语义感知分块
│   └── scopes.ts --------- 作用域隔离
├── 检索层 Retrieval
│   ├── retriever.ts ------ 八级检索管线
│   ├── adaptive-retrieval.ts - 自适应查询过滤
│   ├── noise-filter.ts --- 正则噪声过滤
│   ├── noise-prototypes.ts - embedding 噪声检测
│   ├── decay-engine.ts --- Weibull 衰减计算
│   └── tier-manager.ts --- 三级记忆层管理
├── 提取层 Extraction
│   ├── smart-extractor.ts  - LLM 驱动记忆提取
│   ├── extraction-prompts.ts - 提取 prompt 模板
│   ├── memory-categories.ts - 六分类体系
│   └── smart-metadata.ts -- L0/L1/L2 元数据
└── 集成层 Integration
    ├── index.ts ---------- 3500 行编排中心
    ├── tools.ts ---------- 6 个 Agent 工具
    ├── cli.ts ------------ 命令行管理
    └── reflection-*.ts --- 反思子系统 (4000+ 行)
```
...
三大设计原则本身就是一系列取舍的结果。**生产级稳定性**意味着代码中充斥着大量的 try-catch、超时控制和日志记录，这些"防御性代码"显著增加了代码量（3500 行的 index.ts），但换来的是系统在异常情况下的可预测行为。团队否定了"只处理 happy path"的轻量化方案，因为在 7x24 运行的系统中，异常不是例外而是常态。**优雅降级**的代价是每个子系统都需要暴露"能力状态"接口，增加了模块间的协议复杂度；但否定了"全有或全无"的设计，因为部分功能可用远好于完全不可用。**向后兼容**消耗了 700+ 行迁移代码，且每次新增字段都要维护默认值逻辑；团队评估过"强制迁移、不保留旧格式"的方案，但考虑到用户积累的记忆数据不可再生，最终选择了更保守的渐进式升级路径。

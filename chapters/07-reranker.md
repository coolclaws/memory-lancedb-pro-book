...
+----------+    +------------+    +-----------+    +----------+
| 融合后    +--->+ 选择       +--->+ 构建请求   +--->+ API 调用  |
| 候选集    |    | Provider   |    | (query,   |    | Reranker |
| (25-40条) |    | (6 种)     |    | documents)|    | Service  |
+----------+    +------------+    +-----------+    +-----+----+
                                                        |
                                                   [成功] | [失败]
                                                        v       v
...
Reranker 提供商统一抽象:
              ...
              rerank(query, candidates, config)
                          |
              +-----------+-----------+
              |     Provider Router   |
              +--+--+--+--+--+--+----+
    ...
    |   +--------+  |  |  +--+-------+       |
    v   v        v  v  v             v       v
  Jina Silicon  Voyage Pinecone  DashScope  TEI
  Flow                           (阿里云)   (自托管)
    |     |       |      |         |         |
    v     v       v      v         v         v
  Bearer Bearer Bearer Api-Key  Bearer    可选认证
  token  token  token  header   token

  响应格式差异由 Provider Router 内部处理,
  对外统一返回 {index, score}[]
```

### 设计取舍

**60/40 得分混合比例**的替代方案包括：完全信任 rerank（100/0）、等权混合（50/50）、或可配置比例。完全信任 rerank 在 cross-encoder 对特定领域文本判断出现偏差时没有纠错机制；等权混合则没有充分利用 cross-encoder 更高精度的优势。60/40 是一个"高度信任但留有安全裕度"的折中。另外，**余弦回退**的设计也是一个关键取舍：替代方案是在 API 失败时直接跳过重排序（使用 fusedScore 作为最终分数）。选择余弦回退而非直接跳过，是因为后续管线期望每个候选都经过了精排阶段的调整——余弦回退虽然质量不如 cross-encoder，但提供了一个一致的分数调整语义，避免了管线行为在有无重排序时的分裂。
